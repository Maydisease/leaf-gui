...
#[inline]
 fn log_request(
    ...
    let host = sess.destination.domain().unwrap();

    // let rt = Runtime::new().unwrap();  // 创建一个新的 Runtime
    // rt.block_on(async {
    ...
    //             }
    //         },
    //         Err(e) => println!("Cannot resolve domain: {}", e),
    //     }
    // });
            ...
            T: 'static + AsyncRead + AsyncWrite + Unpin + Send + Sync,
    {
        ...
        match th.handle(&sess, Some(&mut lhs), stream).await {
            Ok(mut rhs) => {
                ...
                let result = self.dns_client.clone()
                    .read()
                    .await
                    .direct_lookup(host).await;
                match result {
                    Ok(addrs) => {
                        for addr in addrs {
                            println!("Resolved Address: {}", addr);
                ...
                //     }
                // } else {
                //     println!("No valid domain provided");
                // }

                // 创建 FakeDns 实例，使用 Include 模式
                let fake_dns = FakeDns::new(FakeDnsMode::Include);
