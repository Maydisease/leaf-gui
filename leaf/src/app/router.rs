...
use crate::config::router::rule::Domain;
use crate::session::{Network, Session, SocksAddr};

use crate::easy_custom;
...
impl Router {
    pub fn add_rule(rules: &mut Vec<Rule>, domains: Vec<Domain>, new_domain_name: &str) {
        let mut new_domains = domains.clone();
        let mut cond_and = ConditionAnd::new();
        new_domains[0].value = String::from(new_domain_name);

        if new_domains.len() > 0 {
            cond_and.add(Box::new(DomainMatcher::new(&mut new_domains)));
        }
        rules.push(Rule::new("Fallback".to_string(), Box::new(cond_and)));
    }
    ...
    fn load_rules(rules: &mut Vec<Rule>, routing_rules: &mut Vec<config::router::Rule>) {
        ...
        for item in fallback_list {
            let first_rule = routing_rules[1].clone();
            Self::add_rule(rules, first_rule.domains, item);
        }
    }
